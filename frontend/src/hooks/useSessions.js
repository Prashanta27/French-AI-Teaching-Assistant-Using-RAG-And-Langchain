...
export function useSessions() {
  ...
  /**
   * Appends `delta` to the content of the LAST message in the active
   * session, leaving every other field untouched. Used while a
   * streaming answer is arriving: call `addMessage("assistant", "")`
   * once to create the empty bubble, then call this repeatedly as
   * chunks arrive from askQuestionStream.
   *
   * No-ops safely if the active session has no messages yet.
   */
  const appendToLastMessage = useCallback((delta) => {
    if (!delta) return;
