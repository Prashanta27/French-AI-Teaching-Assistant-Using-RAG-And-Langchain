...
export default function App() {
  ...
  // This also fires on every streamed chunk (messages array identity
  // changes each time), so the view keeps following the answer as it grows.
  useEffect(() => {
  ...
  const send = async (question) => {
    ...
    // We don't create the assistant bubble until the first chunk
    // arrives -- that way the "typing…" indicator stays on screen
    // during retrieval + the model's time-to-first-token, and the
    // moment real text shows up it replaces the indicator instead of
    // sitting awkwardly next to an empty bubble.
    let started = false;
    ...
    await askQuestionStream(
      ...
      (chunk, { done }) => {
        ...
        if (done) {
          ...
          // Nothing ever streamed in (immediate error, e.g. backend
          // unreachable) -- fall back to showing whatever message we got.
          if (!started) {
            addMessage("assistant", chunk || "No answer returned.");
          }
        }
      },
